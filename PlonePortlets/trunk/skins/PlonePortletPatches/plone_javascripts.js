...
function togglePortlet(pt){
    ...
    if (typeof(pt) == 'string'){
        ...
        icon = portlet.getElementsByTagName('IMG')[0]
        node = icon
    } else if (!pt.nodeType){
    ...
    if (! portlet){var portlet = node.parentNode.parentNode.parentNode}
    
    for (var f = 0; f < portlet.childNodes.length; f++) {
         child = portlet.childNodes[f]
         if (child.nodeName=='DIV'){
      
         if(child.className ==('portletBody')){portletContents=child;}
