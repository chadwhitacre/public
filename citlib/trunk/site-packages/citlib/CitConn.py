...
from citlib.utils import CitError, bitbucket
...
class CitConn:
    ...
    def _readlisting(self):
        ...
        while rawline != '000\n':
            parsedline = self._parse_listing(rawline)
            listing.append(parsedline)
            rawline = self._readline()
        return tuple(listing)


    def _parse_listing(self, line):
    ...
    _LKRN_config_names = ( 'QR_PERMANENT'
                         , 'QR_PRIVATE'
                         , 'QR_PASSWORDED'
                         , 'QR_GUESSNAME'
                         , 'QR_DIRECTORY'
    ...
    _LKRN_config2_names = ( 'QR2_SYSTEM'
                          , 'QR2_SELFLIST'
                           )
    _LKRN_access_names = ( 'UA_KNOWN'
                         , 'UA_GOTOALLOWED'
                         , 'UA_HASNEWMSGS'
                         , 'UA_ZAPPED'
                          )
    def LKRN(self, floor=-1):
        ...
        for raw in self._readlisting():
            room = [None] * 6
            room[0] = raw[0]
            room[1] = bitbucket(raw[1], self._LKRN_config_names)
            room[2] = bitbucket(raw[2], self._LKRN_config2_names)
            room[3] = int(raw[3])
            room[4] = int(raw[4])
            room[5] = bitbucket(raw[5], self._LKRN_access_names)
            rooms.append(room)
