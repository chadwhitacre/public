...
from citlib.utils import CitError, Bucket, bitbucket
...
class CitConn:
    ...
    def LKRN(self, floor=-1):
        ...
        _rooms = self._readlisting()
        for raw in _rooms:
            room = Bucket()
            room.name = v[0]

            config_names = ( 'QR_PERMANENT'
                           , 'QR_PRIVATE'
                           , 'QR_PASSWORDED'
                           , 'QR_GUESSNAME'
                           , 'QR_DIRECTORY'
            ...
            room.config = bitbucket(raw[1], config_names)
            import pdb; pdb.set_trace()
            room.config2 = bitbucket(raw[2])
