...
from zpt._zope.interface.adapter import AdapterRegistry
import zpt._zope.interface

class IF0(zpt._zope.interface.Interface):
    pass
class IF1(IF0):
    pass

class IB0(zpt._zope.interface.Interface):
    pass
class IB1(IB0):
    pass

class IR0(zpt._zope.interface.Interface):
    pass
class IR1(IR0):
    pass
...
def test_orderwith():
    """
    >>> Interface = zpt._zope.interface.Interface
    ...
    >>> [value for spec, value in zpt._zope.interface.adapter.orderwith(bywith)]
...
def test_multi_adapter_w_inherited_and_multiple_registrations():
    ...
    >>> class IX(zpt._zope.interface.Interface):
    ...    pass
...
def test_adapter_hook_with_factory_producing_None():
    ...
    ...     zpt._zope.interface.implements(IF0)
...
def test_adapter_registry_update_upon_interface_bases_change():
    ...
    ...     factory = lookup(zpt._zope.interface.providedBy(ob), iface)
    ...     if factory is None:
    ...         return None
    ...     else:
    ...         return factory(ob)

    >>> zpt._zope.interface.interface.adapter_hooks.append(_hook)
    ...
    >>> class IY(zpt._zope.interface.Interface):
    ...   pass
    ...
    >>> class Y(object):
    ...  zpt._zope.interface.implements(IY)
    ...
    TypeError: ('Could not adapt',
                <zpt._zope.interface.tests.test_adapter.X object at ...>,
                <InterfaceClass zpt._zope.interface.tests.test_adapter.IY>)

    But after we declare an interface on the class `X`, it should pass:

    >>> zpt._zope.interface.classImplementsOnly(X, IX)

    >>> IY(X()) #doctest: +ELLIPSIS
    <zpt._zope.interface.tests.test_adapter.Y object at ...>

    >>> hook = zpt._zope.interface.interface.adapter_hooks.pop()
    """
...
def test_changing_declarations():
    ...
    >>> registry.lookup([zpt._zope.interface.implementedBy(C)], I2, '')

    >>> zpt._zope.interface.classImplements(C, I1)
...
def test_suite():
    from zpt._zope.testing import doctest, doctestunit
