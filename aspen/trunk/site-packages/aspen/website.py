...
class HandlerRuleSet:
    ...
    def __init__(self, rulefuncs, handler):
...
class Website:
    ...
    def __respond_unsafely(self, request):
        ...
        # Look for a resource, and a handler for it.
        # ==========================================

        fspath = self.__static.translate(request.path)
        if not exists(fspath):
            raise Response(404)
        if fspath == self.paths.__: # protect magic directory
            raise Response(404)

        fspath = request.fspath = self.__static.find_default(fspath)
        handler = self.__get_handler(fspath)


        # Set up the contexts and then call the handler.
        # ==============================================
        # session and page/conversation (SEAM) contexts should be added by hooks

        contexts = dict(website=self)               # eternal
        contexts['request'] = request               # ephemeral
        ...
        for line in fp:
            ...
            if not line:                            # blank line
                ...
                if not line.endswith(']'):
                    raise HooksConfigError("missing end-bracket", lineno)
                hook = line[1:-1]
                if hook and (hook not in hooks):
                    raise HooksConfigError("bad hook '%s'" % hook, lineno)
                continue
            elif hook is None:                      # anonymous section
                at_least_one = False
                obj = self.__import(line, HooksConfigError, lineno)
                for name in hooks:
                ...
                dotted = line
                callable_ = self.__import(dotted, HooksConfigError, lineno)
    ...
    def handle(self, **contexts):
        """If nobody else wants it, we will serve it statically.
        """
        return self.__static.respond(contexts['request'])
    ...
    def __load_rulesets(self):
        ...
        a newline-separated list of white-space-separated rule name/dotted name
        pairs. The rule names can be any string without whitespace. Each dotted
        name must specify a Python class, instance, or  module with a callable
        named 'rule', that takes a file object (positioned at 0) and returns a
        boolean. Classes will be instantiated with the website instance as a
        positional argument.
        ...
        Here's an example, which is equivalent to aspen's default behavior:
        ...
        for line in fp:
            ...
            if not line:                            # blank line
                ...
                if not line.endswith(']'):
                    raise HandlersConfigError("missing end-bracket", lineno)
                dotted = line[1:-1]
                handler = self.__import(dotted, HandlersConfigError, lineno)
                ruleset = HandlerRuleSet(rulefuncs, handler)
                rulesets.append(ruleset)
                continue
            elif ruleset is None:                   # anonymous section
                rulename, name = line.split(None, 1)
                obj = self.__import(name, HandlersConfigError, lineno)
                if not hasattr(obj, 'rule'):
                    msg = "rule object at %s has no 'rule' callable" % dotted
                elif not callable(obj.rule):
                    msg = "'rule' attribute of %s is not callable" % dotted
                if msg:
                    raise HandlersConfigError(msg, lineno)
...
this should go in aspen.rules.svn_prop
