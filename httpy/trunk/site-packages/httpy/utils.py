...
__all__ = ('translate', 'Path', 'parse_body', 'parse_cookie', 'parse_query')
...
def couple(responder):
    ...
    """
    from httpy import couplers # Delayed to avoid circular import.

    if os.environ.has_key("GATEWAY_INTERFACE"):
        Coupler = couplers.CGI
    elif os.environ.has_key("FCGI_WEB_SERVER_ADDRS"):
        Coupler = couplers.FastCGI
    else:
        Coupler = couplers.StandAlone
...
class Path(unicode):
    ...
    This is intended to be used as a Response.body, so that couplers can do
    something smart, like use sendfile() if it is available.
