...
def uri_to_fs(config, resource_uri_path, defaults=[], raw=False):
    ...
    if config.app_fs_root == config.site_root:
        _parts = resource_uri_path.lstrip('/').split(os.sep)
        resource_fs_path = os.path.join(config.site_root, *_parts)
    else:
        uri_below_app = resource_uri_path[len(config.app_uri_root):]
        _parts = uri_below_app.lstrip('/').split(os.sep)
        resource_fs_path = os.path.join(config.app_fs_root, *_parts)
