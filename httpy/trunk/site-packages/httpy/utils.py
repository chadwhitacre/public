...
def load_app(site_root, app_uri_root):
    ...
    logger = logging.getLogger('httpy.load_app')
    ...
    if __ is None:
        logger.debug("Falling back to DefaultApp for %s" % site_root)
        app = DefaultApp
    else:
        logger.debug("Loading app from %s" % __)
        try:
    ...
    try:
        ...
        logger.debug("app in %s is not valid." % fs_root)
        raise
    try:
        verifyClass(IApplication, app.Application, tentative=True)
    except:
        logger.debug("app.Application in %s is not valid." % fs_root)
        raise
...
def uri_to_fs(site_root, app_fs_root, app_uri_root, resource_uri_path,
    ...
    logger = logging.getLogger('httpy.uri_to_fs')
    ...
    if os.path.isdir(resource_fs_path):
        ...
        if not default:
            logger.debug("No default resource in %s" % resource_uri_path)
            raise Response(403)
...
if 0: # deprecating this in favor of logging pkg from stdlib
