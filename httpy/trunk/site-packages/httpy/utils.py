...
class Gaoler:
    ...
    Gaoler takes over sys.modules, and removes all modules which are loaded by
    code run under its control. The result is that next time the code is run,
    the modules will be reloaded from the filesystem.
