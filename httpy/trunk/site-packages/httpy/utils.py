...
class Gaoler:
    """Useful for running some python within a sandbox environment.

    Gaoler takes over sys.path and sys.modules, so that you can import and use
    modules without affecting the global module-space.

        >>> import Gaoler
        >>> gaoler = Gaoler()
        >>> gaoler.capture('path/to/custom/site-packages')
        >>> # do something here
        >>> gaoler.release()
    ...
    def capture_path(self, directory):
        self._path = None
        if directory not in sys.path:
            log(98, "Capturing sys.path.")
            self._path = sys.path[:]
            sys.path.insert(0, directory)

    def capture_modules(self):
        log(98, "Capturing sys.modules.")
    ...
    def release_path(self):
        if self._path:
            sys.path = self._path
            log(98, "Restored sys.path.")

    def release_modules(self):
        for module in sys.modules.keys():
            if module not in self._modules:
                del(sys.modules[module])
        log(98, "Restored sys.modules.")
