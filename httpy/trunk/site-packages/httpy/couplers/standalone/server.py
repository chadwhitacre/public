...
class Server(ServerBase):
    ...
    def start(self):
        ...
        self.create_socket(self.config.sockfam, socket.SOCK_STREAM)
        self.set_reuse_addr()
        if (not WINDOWS) and (self.config.sockfam == socket.AF_UNIX):
        ...
        if self.config.sockfam == socket.AF_INET:
            ...
            # Update responder.uri
            # ====================

            if self.responder.uri is None:
                p = ''
                if port != '80':
                    p = ':%s' % port
                self.responder.uri = "http://%s%s/" % (addr, p)


        elif (not WINDOWS) and (self.config.sockfam == socket.AF_UNIX):
