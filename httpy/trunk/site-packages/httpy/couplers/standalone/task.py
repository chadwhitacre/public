...
from httpy.utils import uri_to_fs, FileWrapper
...
class Task:
    ...
    def deliver(self, response):
        ...
        if (response.code != 304) and (self.request.method != 'HEAD'):
            ...
            elif isinstance(response.body, FileWrapper):
                if SENDFILE:
                    self.channel.flush()
                    fd = self.channel.fileno()
                    sendfile.sendfile( self.channel.fileno()
                                     , response.body.fileno
                                     , 0
                                     , 0
                                      )
                else:
