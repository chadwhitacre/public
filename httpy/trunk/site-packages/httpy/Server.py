...
class Server(ServerBase):
    ...
    def __init__(self, config, threads=10):
        ...
        self.logger = logging.getLogger('httpy.Server')
        ...
        if config.sockfam == socket.AF_UNIX:
            if os.path.exists(config.address):
                ...
                self.logger.debug("unlinking stale socket")
    ...
    def start(self):
        ...
        if self.config.sockfam == socket.AF_INET:
            ...
            self.logger.info("httpy starting on port %s" % port)
        elif self.config.sockfam == socket.AF_UNIX:
            self.logger.info("httpy starting on %s" % self.config.address)
        asyncore.loop(timeout=5)


    def stop(self, signum=None, frame=None):
        if signum is not None:
            self.logger.debug("caught %s" % STOP_SIGNALS[signum])
        self.logger.info("shutting down...")

        for app in self.config.apps:
            if hasattr(app, 'close') and inspect.ismethod(app.close):
                self.logger.debug("closing %s" % app)
                app.close()
            else:
                self.logger.debug("%s provides no close hook" % app)
        ...
        if self.config.sockfam == socket.AF_UNIX:
            if not os.path.exists(self.config.address):
                self.logger.debug("what happened to our socket?")
            os.unlink(self.config.address)

        self.logger.info("httpy stopped")
...
class RestartingServer:
    ...
    def __init__(self, config):
        ...
        for sig in STOP_SIGNALS:
           signal.signal(sig, self.stop)
        self.logger = logging.getLogger('httpy.RestartingServer')


    def start(self, args=None):
        self.logger.info("starting child server ...")
        if args is None:
        ...
        while 1:
            try:
                ...
                self.logger.info("Restarting child server ...")
            except:
                self.logger.error("Exception while spawning child ...\n" +
    ...
    def stop_child(self):
        ...
        try:
            ...
            self.logger.warning("OSError killing pid %d" % self.pid)
    ...
    def stop(self, signum=-1, frame=None):
        ...
        self.stop_child()
        if signum is not None:
            self.logger.debug("parent caught %s" % STOP_SIGNALS[signum])
        self.logger.debug("parent shutting down...")
        raise SystemExit
