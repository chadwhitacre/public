...
class Server(ServerBase):
    ...
    def __init__(self, config, threads=10):
        ...
        if config.sockfam == socket.AF_INET:
            self.set_reuse_addr()
        elif config.sockfam == socket.AF_UNIX:
            if os.path.exists(config.address):
                os.unlink(config.address)
                log(97, "unlinking stale socket")
    ...
    def start(self):
        ...
        if self.config.sockfam == socket.AF_INET:
            ...
            log(1, "httpy starting on port %s" % port)
        elif self.config.sockfam == socket.AF_UNIX:
            log(1, "httpy starting on %s" % self.config.address)
        log(99, "%s\n\n" % ("="*76))
