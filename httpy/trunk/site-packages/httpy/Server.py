...
class Server(ServerBase):
    ...
    def start(self):
        ...
        if self.config.sockfam == socket.AF_INET:
            ...
            logger.info("httpy starting on port %s" % port)
        elif self.config.sockfam == socket.AF_UNIX:
            logger.info("httpy starting on %s" % self.config.address)
    ...
    def look_for_changes(self):
        ...
        for module in sys.modules.values():
            ...
            if not os.path.isfile(filename):
                logger.debug("File is missing: %s" % filename)
            ...
            if mtime > self.mtimes[filename]:
                logger.debug("File is outdated: %s" % filename)
                self.mtimes[filename] = mtime
                raise Restart
