...
from httpy.apps.Default import DefaultApplication
from httpy.interfaces import IApp, IApplication, IFramework
from httpy.frameworks import DefaultFramework
...
class Loader:
    ...
    def get_framework(self):
        ...
        try:
            ...
            logger.info("falling back to DefaultFramework")
...
class Server(ServerBase, Loader):
    ...
    def __init__(self, config):
        ...
        # Bind to an address, then switch users.
        # ======================================
        ...
        while 1:
            if time.time() < timeout:
                ...
                logger.error('could not connect to %s' % str(config.address))
                raise err

        if self.config.user:
            uid, gid = pwd.getpwnam(self.config.user)[2:4]
            groupname = grp.getgrgid(gid)[0]
            logger.debug("setting uid/gid for process to %d:%d " % (uid, gid) +
                         "(%s:%s)" % (self.config.user, groupname))
            os.setregid(gid, gid)
            os.setreuid(uid, uid)
    ...
    def stop(self, signum=None, frame=None):
        ...
        if self.config.sockfam == socket.AF_UNIX:
            if os.path.exists(self.config.address):
