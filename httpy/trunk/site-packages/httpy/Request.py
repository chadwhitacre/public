...
class Request:
    ...
    def set_line(self, stream):
        ...
        if len(tokens) == 3:
            ...
            raise Endings.BadRequest("The Request-Line `%s' appears " % line +
        ...
        if method not in ('GET', 'HEAD', 'POST'):
            raise Endings.NotImplemented("This server does not implement " +
                                          "the `%s' method." % method)
        ...
        if not m:
            raise Endings.BadRequest("The HTTP-Version `%s' " % version +
    ...
    def set_body(self, stream):
        ...
        content_length = int(self.message.get('Content-Length'))
        ...
        # raising BadRequest. I suppose the concern is that the input stream
...
keep alive --> app?
range requests --> app, with response fallback
