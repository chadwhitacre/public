...
class Request:
    ...
    wire, so they are possible to get. raw also includes an initial blank lines.
    ...
    def parse_line(self):
        ...
        tokens = self.line.rstrip('/n').rstrip('/r').split()
        if len(tokens) == 3:
    ...
    def get_headers(self, block):
        ...
        for newline in ('\r\n', '\n', '\r'):
            if newline*2 in block:
                self.headers, self._tmp = block.split(newline*2, 1)
    ...
    def set_receiver(self):
        ...
        self.received, since it actually holds data between such calls.
    ...
    def get_body(self, block):
        ...
        if self.method in ('POST', 'PUT'):
            self.body = self._receiver.getfile().read()
        else:
            self.body = ''
        raise Complete
