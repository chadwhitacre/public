...
from httpy.responders.multiple.interfaces import Iresponder, IResponder
from httpy.responders.multiple.interfaces import Iframework, IFramework
from httpy.responders.multiple.frameworks import PassThrough
...
class Responder:
    ...
    def respond(self, request):
        ...
        # Get a responder and update request.path accordingly.
        # ====================================================

        responder = self.get_responder(request)
        #request.path_translated = utils.translate( request.path
        #                                         , responder.root
        #                                         , raw=True
        #                                          )
        #request.path_info = request.path[len(responder.uri)-1:]


        # Inbound hook #2.
        # ================

        request = self.framework.wrap_request(request, responder)


        # Call and outbound hook.
        # =======================

        try:
            responder.respond(request)
        except (Response, CustomResponse), response:
            raise self.framework.unwrap_response(response, request, responder)
        except:
            raise
    ...
    def stop(self):
        ...
        for responder in self.responders:
            if hasattr(responder, 'close'):
                if inspect.ismethod(responder.close):
    ...
    def get_framework(self):
        ...
        try:
            ...
            logger.info("falling back to pass-through")
    ...
    def load_responder(self, root):
        ...
        sys.modules. We also insert exactly one of the following paths into
