    ...
    def get_responder(self, request):
        ...
        for _responder in self.responders:
            if request.path.startswith(_responder.path):
                ...
                request.path = request.path[len(responder.path):]
                if not request.path:
                    request.path = '/'
                break
        ...
        if not os.path.isdir(responder.root):
            logger.critical( "Filesystem root has disappeared for " +
        ...
        if responder.__ is not None:
            if not os.path.isdir(responder.__):
                logger.critical( "Magic directory has disappeared for " +
                                 "responder '%s'" % responder
                                )
                raise Response(500)
        ...
        if responder.pkg is not None:
            if not os.path.isdir(responder.pkg):
                logger.critical( "Custom package directory has disappeared " +
                                 "for responder '%s'" % responder
                                )
                raise Response(500)
