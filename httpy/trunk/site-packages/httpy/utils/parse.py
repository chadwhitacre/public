...
#
# While httpy.couplers.standalone.request keeps close to the HTTP layer, any
# dynamic application will need to comprehend application-specific information
# encoded in the Request. The functions below return representations of such
# information as objects from the standard library.
#
#   function            uses                            returns
#   =======================================================================
#   parse_body          raw_body                        cgi.FieldStorage
#   parse_basic_authen  headers.get('Authorization')    (username, passwor)
...
def parse_basic_authen(request):
    """Given a Request, return the username and password.
    """
    auth = request.headers.get('Authorization', '')
    if auth:
        scheme, param = auth.split(None, 1)
        param = base64.b64decode(param)
        username, password = param.split(':', 1)
    else:
...
def parse_fieldstorage(fieldstorage, encoding=None):
    ...
    XXX: shouldn't encoding be handled by parse_* per the content-type?
