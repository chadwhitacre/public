...
There are three classes here:
    ...
    Loader -- provides framework and application loading
...
from httpy.apps import Static
from httpy.channel import Channel
from httpy.interfaces import Iapp, IApp, ILoader, IFramework
from httpy.frameworks.default import Framework as DefaultFramework
...
class Server(ServerBase):
    ...
    response_header = "httpy/0.8"


    def __init__(self, config, framework=None):

        self.config = config
        self.framework = framework or DefaultFramework
        try:
            verifyObject(IFramework, self.framework, tentative=True)
        except:
            logger.debug("%s is not valid." % self.framework)
            raise
