...
from httpy.response import Response, ResponseFlag, StatusCodes
...
class Task:
    ...
    def __init__(self, channel, request):
        ...
        try:
            ...
            self.app        = self.framework.get_app()
    ...
    def respond(self):
        ...
        The call to App.respond is complicated by the fact that in debugging
        mode we want to drop into the post-mortem debugger when there is an
        exception (other than Response, of course).

        App.respond is expected to raise a Response or other exception.

        """

        # Get out of the way.
        # ===================
        # This is where the inbound framework hook is.

        request = self.framework.wrap_request(self.app, self.request)

        if not httpy.mode.DEBUG:
        ...
        raise Response(500, "%s.respond did not raise " % str(self.app) +
                            "anything.")
    ...
    def deliver(self, response):
        ...
        elif (response.code == 537) and httpy.mode.DEPLOYMENT:
