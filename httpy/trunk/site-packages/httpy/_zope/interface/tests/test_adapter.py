...
$Id: test_adapter.py 37594 2005-07-30 13:32:05Z jim $
"""
import unittest
import httpy._zope.interface
from httpy._zope.interface.adapter import AdapterRegistry
import httpy._zope.interface

class IF0(httpy._zope.interface.Interface):
    pass
class IF1(IF0):
    pass

class IB0(httpy._zope.interface.Interface):
    pass
class IB1(IB0):
    pass

class IR0(httpy._zope.interface.Interface):
    pass
class IR1(IR0):
    pass
...
def test_orderwith():
    """
    >>> Interface = httpy._zope.interface.Interface
    ...
    >>> [value for spec, value in httpy._zope.interface.adapter.orderwith(bywith)]
...
def test_multi_adapter_w_inherited_and_multiple_registrations():
    ...
    >>> class IX(httpy._zope.interface.Interface):
    ...    pass
...
def test_adapter_hook_with_factory_producing_None():
    ...
    ...     httpy._zope.interface.implements(IF0)
...
def test_adapter_registry_update_upon_interface_bases_change():
    ...
    ...     factory = lookup(httpy._zope.interface.providedBy(ob), iface)
    ...     if factory is None:
    ...         return None
    ...     else:
    ...         return factory(ob)

    >>> httpy._zope.interface.interface.adapter_hooks.append(_hook)
    ...
    >>> class IY(httpy._zope.interface.Interface):
    ...   pass
    ...
    >>> class Y(object):
    ...  httpy._zope.interface.implements(IY)
    ...
    TypeError: ('Could not adapt',
                <httpy._zope.interface.tests.test_adapter.X object at ...>,
                <InterfaceClass httpy._zope.interface.tests.test_adapter.IY>)

    But after we declare an interface on the class `X`, it should pass:

    >>> httpy._zope.interface.classImplementsOnly(X, IX)

    >>> IY(X()) #doctest: +ELLIPSIS
    <httpy._zope.interface.tests.test_adapter.Y object at ...>

    >>> hook = httpy._zope.interface.interface.adapter_hooks.pop()
    """
...
def test_changing_declarations():
    ...
    >>> registry.lookup([httpy._zope.interface.implementedBy(C)], I2, '')

    >>> httpy._zope.interface.classImplements(C, I1)
...
def test_suite():
    from httpy._zope.testing import doctest, doctestunit
