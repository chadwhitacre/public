...
doing we demonstrate how to construct an httpy application. We also provide a
mixin class with useful API for more dynamic applications.
...
class Transaction:
    ...
    def set_path(self, request):
        ...
        if not request.path:
            # this catches, e.g., '//foo' -- although now urlsplit is in Request
