...
class Config(dict):
    ...
    def set_apps(self):
        """Find apps within the website root.

        This method is only run if the user has not explicitly configured apps.
        Users can set apps to the empty string in order to turn off apps
        altogether.

        This probably violates EIBTI. Our rationale is based on our use case: we
        plan on automatically deploying hundreds of httpy sites. In part this
        means automatically configuring each httpy process, so we could push
        this method out to that layer. However, this method represents enough
        magic that I want to get used to it happening in development so that I
        can trust it in production.

        For the other parameters, by contrast, either the defaults are fine (ip,
        mode, verbosity), or they are simple enough that configuring them
        automatically from our production environment is safe (port, root).

        Another option would be to stick apps in a svn property on the site
        root, and use that from our auto-deployment tools.

        And furthermore, the apps parameter is the only one internal to the
        site; the rest are really meta-.
    ...
    def validate_apps(self):
        ...
        We break this out so that self['root'] is set first.
        ...
        for app_path in self['apps']:
            ...
            __ = os.path.join(full_path, '__')
            if (not os.path.isdir(full_path)
                or not os.path.isdir(__)):
