...
import os
import socket
from ServerConfigParser import RawServerConfigParser
...
class AppServerConfig:
    """Represents an httpy application's configuration.
    """
    pass
...
class ServerServerConfig:
    ...
    values are coerced to the requisite types; if not, ServerConfigError is raised.
    ...
    The `ossify' method is used to convert a ServerConfig object to a standard
    dictionary.
    ...
    def __init__(self, argv=[]):
        ...
        # Locate, validate, and load this site's applications.
        # ====================================================
    ...
    def _opts(self, argv):
        ...
        if opts.path:
            ...
            if not os.path.isfile(path):
                raise ServerConfigError("The path %s does not point to a "
                                  "file." % path)
    ...
    def _validate(self, context, d):
        ...
        checked. If either fails, ServerConfigError is raised.
    ...
    def _validate_apps(self):
        ...
        We break this out so that self['root'] is set first. self['apps'] is
        already guaranteed by self._validate and/or self.set_apps to be a tuple
        of strings. Our job here is to make sure that the apps exist on the
        filesystem and that each has a magic directory.

        This is also where we guarantee that the website root is always
        considered an application (we special-case it later if it has no magic
        directory).
        ...
        for app_uri_path in self.apps:
            app_fs_root = os.path.join(self.root, app_uri_path.lstrip('/'))
            app_fs_root = os.path.realpath(app_fs_root)
            __ = os.path.join(app_fs_root, '__')

            if not os.path.isdir(app_fs_root):
                raise ServerConfigError(msg % app_fs_root)

            if not os.path.isdir(__):
                if app_fs_root != self.root:
                    raise ServerConfigError(msg % app_fs_root)
    ...
    def _load_apps(self):
        """Load the app modules into a list of tuples.
        """
        apps = []
        for app_uri_root in self.apps:
            app = AppServerConfig()
            app.uri_root = app_uri_root
            app.fs_root = os.path.join(self.root, app_uri_root.lstrip('/'))
            app.__ = os.path.join(app.fs_root, '__')
            app.module = self._load_app(app.__)
            apps.append(app)
        self.apps = apps


    def _load_app(self, __)
        """Given a magic directory, return a module.

        For the root directory, we support automatically falling back to
        DefaultApp. Otherwise, we fail if there is no importable app.

        """

        if __ is None: # guaranteed by ServerConfig to only be true for the root
            app = DefaultApp
            ...
            try:
                ...
                print "There was an error importing an app from %s:" % __
                raise
