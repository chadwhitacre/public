...
class Config:
    ...
    def __init__(self, argv=[]):
        ...
        # why is mode in the env? is this leftover from the old logging mech?
    ...
    def _opts(self, argv):
        ...
        parser_.add_option("-m", "--mode", dest="mode",
                           help="a string indicating the server mode. " +
                                "[deployment]")
    ...
    def _validate(self, context, d):
        ...
        if d.has_key('threads'):

            msg = ("Found bad thread count `%s' in context `%s'. " +
                   "Threads must be an integer greater than or equal to one.")
            msg = msg % (str(d['threads']), context)

            if not isinstance(d['threads'], (int, long)):
                isstring = isinstance(d['threads'], basestring)
                if not isstring or not d['threads'].isdigit():
                    raise ConfigError(msg)
