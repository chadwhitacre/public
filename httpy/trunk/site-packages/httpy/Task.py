...
class Task:
    ...
    def __init__(self, channel, request):
        ...
        self.request = self.scrub_request(request)
    ...
    def scrub_request(self, request):
        """Remove unwanted API from the request.
        """
        scrubbed = CleanRequest()
        for attr in ['raw', 'raw_line', 'raw_headers', 'raw_body',
                     'method', 'uri', 'path', 'message']:
            val = getattr(request, attr)
            setattr(scrubbed, attr, val)
        return scrubbed
