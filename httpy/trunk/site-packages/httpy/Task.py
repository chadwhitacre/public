...
class Task:
    ...
    def process(self):
        ...
        The call to Application.process is complicated by the fact that in
        development mode we want to drop into the post-mortem debugger when
        there is an exception (other than Response, of course).

        Application.process is expected to raise a Response or other exception.

        """

        config = ApplicationConfig(self.app, self.server_config)
        ...
        transaction = self.app.module.Application(config)
    ...
    def respond(self, response):
        ...
        if response.code == 537 and not self.dev_mode:
            raise Exception("Bad response code: %r" % response.code)
        ...
        # Generate the body.
        # ==================
        # We do this hear so we can calculate the content-length.

        if not response.body:
            response.body = reason_message
        if self.dev_mode and response.code == 537:
