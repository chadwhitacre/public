...
class Task:
    ...
    def respond(self):
        ...
        raise Response(500, "%s.respond did not raise " % unicode(application) +
                            "anything.")
    ...
    def deliver(self, response):
        ...
        status_line = ' '.join(( unicode(self.server.http_version_string)
                               , unicode(response.code)
                               , reason_phrase
                                ))
        ...
        if (not self.server.deploy_mode) and (response.code == 537):
        ...
        if 'content-type' not in headers:
            if unicode(response.code).startswith('2'):
