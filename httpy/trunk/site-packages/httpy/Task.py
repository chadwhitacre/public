...
class Task:
    ...
    def get_transaction(self, config, uri_path):
        ...
        if not __:
            ...
            # Create a clean environment for our application.
            # ===========================================================
            # We take over sys.path so that __ functions like site-packages.
            # It is not enough just to pass __ to find_module, since we need
            # it to be in the path of anything that app imports as well.
            #
            # A single website may have multiple applications, and
            # therefore multiple magic directories. We want these to be
            ...
            if config['mode'] == 'development':
                log(99, "Resetting available modules.")
                if self.rollbackImporter:
                    self.rollbackImporter.uninstall()
                self.rollbackImporter = RollbackImporter()
                linecache.clearcache()
                log(95, "Available modules reset.")
            ...
            for modname in sys.modules.keys():
                if modname.startswith('zope'):
                    log(99, "Deleting %s" % modname)
                    del(sys.modules[modname])
            log(95, "zope removed from namespace.")

            # Steve: the next statement -- import app -- is reliably producing a
            # segfault for me
            ...
            # I'm guessing the rest of this could be enforced with interfaces
