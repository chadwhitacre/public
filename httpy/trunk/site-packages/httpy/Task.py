...
from httpy.Response import BaseResponse, Response, StatusCodes
...
class Task:
    ...
    def service(self):
        ...
        try:
            self.respond()
        except BaseResponse, response:
            try:
                response = self.framework.Response(response)
    ...
    def respond(self):
        ...
        # Get out of the way.
        # ===================
        # This is where the framework hooks come in.

        frameworked_request = self.framework.Request(self.request)
        if not self.server.debug_mode:
            self.app.respond(frameworked_request)
        else:
            try:
                self.app.respond(frameworked_request)
            except BaseResponse, response:
                raise self.framework.Response(response)
            except:
