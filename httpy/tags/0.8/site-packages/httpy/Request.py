...
from httpy._zope.server.interfaces import IStreamConsumer
...
class Request:
    ...
    applications. Instances of this class are what are actually passed to
    Application.respond.
...
class ZopeRequest:
    """An implementation of Request that works with the httpy._zope.server machinery.
    """
    ...
    def parse_line(self):
        ...
        if len(tokens) == 3:
            ...
            raise Response(400, "The Request-Line `%s' " % self.raw_line +
        ...
        if not m:
            raise Response(400, "The HTTP-Version `%s' appears to " % version +
    ...
    def get_headers(self, block):
        ...
        Technically MIME messages are \r\n delimited, but we can't assuming
        that. We want to split on the first double line break we get,
        regardless of the control characters used.
