...
class Configure:
    """Determine the config for an httpyd server from three sources:
    ...
    return a 5-tuple:
        ...
        mode        the httpy mode
        paths       an object containing useful paths

    """

    commands_ = ('serve', 'dump')
    options = ( 'address'
              , 'mode'
              , 'threads'
              , 'user'
    ...
    def __call__(self, argv):
        ...
        usage = "http://www.zetadev.com/software/httpyd/"
        ...
        class Paths:
            pass
        paths = Paths()
        paths.root = os.path.realpath(os.getcwd())
        paths.__ = os.path.join(paths.root, '__')
        if not os.path.isdir(paths.__):
            paths.__ = None
            paths.pkg = None
        else:
            paths.pkg = os.path.join(paths.__, 'site-packages')
            if not os.path.isdir(paths.pkg):
                paths.pkg = None
            else:
                sys.path.insert(0, paths.pkg)
        return self.address, self.threads, self.uid, self.mode, paths
