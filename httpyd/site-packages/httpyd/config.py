...
class Configure:
    ...
    def __call__(self, argv):
        ...
        if args:
            command = args[0]
        if command not in self.commands_:
            raise ConfigError('Bad command: %s' % command)
        ...
        site_packages = os.path.join(paths.__, 'site-packages')
        if os.path.exists(site_packages):
            sys.path.insert(0, site_packages)
        return self.address, self.threads, self.uid, self.mode, paths, command
